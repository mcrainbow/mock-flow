...
import type {
  FetchCheckboxQuestionsFromAIParams,
  AICheckboxQuestionsResponse,
} from '@entities/question/model';

export async function fetchCheckboxQuestionsFromAI({
  topic,
  level,
  count,
}: FetchCheckboxQuestionsFromAIParams): Promise<AICheckboxQuestionsResponse> {
  const message = `topic=${topic}, level=${level}, count=${count}`;

  const { data, error } = await supabase.functions.invoke('get_checkbox_questions', {
    body: { message },
  });
  ...
  let questionsData: AICheckboxQuestionsResponse;
  try {
